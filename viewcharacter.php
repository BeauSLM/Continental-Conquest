<?php
    include 'include/header.php';
    include 'include/left-menu.php';
    $char_name = $_POST["char_name"];
    $query_string = "SELECT * FROM CHARACTERS where name =\'".$char_name."\'";
    $result = $db->connection->query($query_string);
?>
                <div class="center">
                    <div id="center-content">
                        <table id="table_players">
                            <tr class="table_players_row">
                                <td class="table_players_data" id="table_players_title" colspan="6" >
                                    Character : <?php if ($result->num_rows == 0) { echo "Character not found."}?>
                                </td>
                            </tr>
                            <tr class="table_players_row">
                                <td class="table_players_header_data">
                                    Name
                                </td>
                                <td class="table_players_header_data">
                                    Player
                                </td>
                                <td class="table_players_header_data">
                                    Guild
                                </td>
                                <td class="table_players_header_data">
                                    Level
                                </td>
                                <td class="table_players_header_data">
                                    Race
                                </td>
                                <td class="table_players_header_data">
                                    Class
                                </td>                         
                            </tr>
                            <tr class="table_players_row">
                                <td class="table_players_data">
                                    JohnDoer
                                </td>
                                <td class="table_players_data">
                                    PapaSmurf
                                </td>
                                <td class="table_players_data">
                                    SmurfVillage
                                </td>
                                <td class="table_players_data">
                                    50
                                </td>
                                <td class="table_players_data">
                                    Elf
                                </td>
                                <td class="table_players_data">
                                    Hunter
                                </td>
                            </tr>
                            <tr class="table_players_row">
                                <td class="table_players_data">
                                    JohnDoer5
                                </td>
                                <td class="table_players_data">
                                    MamaSmurf
                                </td>
                                <td class="table_players_data">
                                    DartPlayers
                                </td>
                                <td class="table_players_data">
                                    35
                                </td>
                                <td class="table_players_data">
                                    Orc
                                </td>
                                <td class="table_players_data">
                                    Ranger
                                </td>
                            </tr>
                            <tr class="table_players_row">
                                <td class="table_players_data">
                                    JohnDoer5
                                </td>
                                <td class="table_players_data">
                                    duckFartMan
                                </td>
                                <td class="table_players_data">
                                    sToRyTeLlErS
                                </td>
                                <td class="table_players_data">
                                    25
                                </td>
                                <td class="table_players_data">
                                    Dark Elf
                                </td>
                                <td class="table_players_data">
                                    Druid
                                </td>
                            </tr>
                            <tr class="table_players_row">
                                <td class="table_players_data" colspan="6">
                                    1 <a href="#">2</a> <a href="#">2</a> <a href="#">3</a>
                                </td>
                            </tr>
                        </table>
                    </div>
                </div>
<?php
    include 'include/right-menu.php';
    include 'include/footer.php';
?>